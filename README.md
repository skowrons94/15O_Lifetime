...
5. **Sliding-window Gaussian fit** — Bin events in overlapping θ windows (30° wide, 7° step); fit a Gaussian to the Δβ distribution in each window to extract its mean.
6. **MCMC Doppler fit** — Fit the θ-dependent Δβ profile with the relativistic Doppler model using `emcee`; free parameters are the energy offset ΔE, angle offset Δθ (optional), and the integrated Δβ. Corner and fit plots are saved to `plots/`.
...
- Outputs median lifetimes with 68% credible intervals; upper limits (90% CL) are quoted for states consistent with zero.
...
`data/15O_Lifetime_AGATA.npy` is a NumPy dictionary (`.item()`) keyed by γ energy (integer keV). Each entry contains per-event arrays:
